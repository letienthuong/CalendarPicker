...
export default class CalendarPicker extends Component {
  ...
  onPressStaticButton = ({ firstDate, secondDate }) => {
    ...
    this.setState(
      ...
      () => {
        onDateChange(firstDate, Utils.START_DATE);
        onDateChange(secondDate, Utils.END_DATE);
        this.handleOnSelectMonthYear({
          month: parseInt(secondDate.format('M')) - 1,
          year: parseInt(secondDate.format('YYYY')),
        });
      },
    );
  };
  ...
  render() {
    ...
    return (
      <GestureRecognizer
        onSwipeRight={this.handleOnPressPrevious}
        onSwipeLeft={this.handleOnPressNext}
        config={{ velocityThreshold: 0.3, directionalOffsetThreshold: 80 }}
      >
        {!hideQuickSelect && allowRangeSelection && allowBackwardRangeSelect && enableDateChange ? (
          <StaticRange
            firstDate={selectedStartDate}
            secondDate={selectedEndDate}
            onPressStaticButton={this.onPressStaticButton}
          />
        ) : null}
        {content}
        {currentView !== 'days' && (
          <TouchableOpacity style={styles.backButton} onPress={this.handleOnPressBack}>
            <Text>Trở về</Text>
          </TouchableOpacity>
        )}
      </GestureRecognizer>
